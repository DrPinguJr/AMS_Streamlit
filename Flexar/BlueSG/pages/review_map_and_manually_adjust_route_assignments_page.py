...
from Flexar.BlueSG.components.register_drag_and_drop_route_assignment_board import ROUTE_BOARD_COMPONENT
...
def run_route_planner_screen_flow() -> None:
    ...
    if existing_state is None:
        ...
        token = st.text_input(
            ...
            help=(
                "Optional manual token. Leave blank to use the configured OneMap "
                "credentials. Used only when you explicitly refresh connectors or apply routes."
            ),
        )
...
with top_cols[1]:
    map_token = st.text_input(
        ...
        help=(
            "Optional manual token. Leave blank to use the configured OneMap "
            "credentials. Used for map geocoding and route recalculation."
        ),
    )
