...
from Flexar.BlueSG.validate_route_assignment_hard_constraints import Constraint, validate_candidate_routes
from Flexar.BlueSG.route_optimisation_result_models import OptimisationRunResult, TravelLegResult
...
ROSTER_FILE = DATA_DIR / "weekday_rider_availability_and_capacity_roster.xlsx"
...
GEOCODE_SEED_CACHE_FILE = CACHE_DIR / "seed" / "verified_onemap_address_coordinates_seed.csv"
GEOCODE_CACHE_FILE = RUNTIME_CACHE_DIR / "onemap_address_coordinates_runtime_cache.csv"
ROUTE_CACHE_FILE = RUNTIME_CACHE_DIR / "onemap_travel_routes_runtime_cache.csv"
...
def _load_route_disk_cache_once() -> None:
    ...
    if ROUTE_DISK_CACHE_LOADED:
        return
    for cache_file in [ROUTE_CACHE_FILE]:
...
def build_driver_route_progress_context(
    route_row: dict[str, Any],
    driver_total_orders: int,
) -> dict[str, Any]:
    """Describe where one order sits within its driver's route."""

    try:
        driver_order_number = max(
            0,
            int(float(route_row.get("Sequence", 0) or 0)),
        )
    except (TypeError, ValueError, OverflowError):
        driver_order_number = 0
    try:
        total_orders = max(0, int(driver_total_orders))
    except (TypeError, ValueError, OverflowError):
        total_orders = 0
    total_orders = max(driver_order_number, total_orders)

    if driver_order_number == 1:
        location_source = "Driver's starting location"
    elif driver_order_number > 1:
        location_source = "Previous order's drop-off location"
    else:
        location_source = "Route starting point"

    return {
        "driver_order_number": driver_order_number,
        "driver_total_orders": total_orders,
        "driver_orders_before_this": max(0, driver_order_number - 1),
        "driver_location_before_order": clean_text(route_row.get("Start From")),
        "driver_location_source": location_source,
    }
...
def optimise_vehicle_routes(
    ...
    while remaining and riders:
        ...
        dropoff_address = clean_text(job["Drop-off Address"])
        lookup_warnings.extend(evaluation.get("warnings", []))
        driver_progress_context = build_driver_route_progress_context(
            inserted_row,
            len(evaluation.get("rows", [])),
        )
    ...
    driver_total_orders_by_name = (
        route_df.groupby("Rider", sort=False).size().astype(int).to_dict()
        if not route_df.empty
        else {}
    )
    for final_index, row in route_df.reset_index(drop=True).iterrows():
        final_rider_name = clean_text(row.get("Rider"))
        final_rider = riders_by_name.get(final_rider_name)
        final_job = all_job_records_by_id.get(int(row.get("Uploaded Row", -1)))
        if (
            final_rider is not None
            and final_job is not None
            and _priority_rider_matches_job(final_rider, final_job)
        ):
        ...
        driver_progress_context = build_driver_route_progress_context(
            row.to_dict(),
            driver_total_orders_by_name.get(final_rider_name, 0),
        )
        report(
            ...
            current_rider=final_rider_name,
            current_pickup=clean_text(row.get("Pickup Address")),
            current_dropoff=clean_text(row.get("Drop-off Address")),
            current_region=clean_text(
                row.get("Operational Subregion") or row.get("Pickup Zone")
            ),
            rider_load_level=(
                normalise_rider_load_level(final_rider.load_level)
                if final_rider is not None
                else ""
            ),
            assignment_score=round(
                float(row.get("Assignment Score", 0) or 0),
                1,
            ),
...
def improve_route_dataframe(
    ...
    *,
    ...
    from Flexar.BlueSG.improve_routes_after_initial_optimisation import improve_assigned_routes
    from Flexar.BlueSG.route_optimisation_metrics_and_run_summary import create_run_result, objective_tuple
